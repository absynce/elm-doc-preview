...
const elmErrorWithColor = (errors) => {
 ...
 */
const debounce = (fn, debounceMilliseconds) => {
 ...
 */
function getElm() {
    let elm = (args, cwd = ".") => spawn.sync("npx", ["--no-install", "elm"].concat(args), { cwd });
...
async function packageReleases(pattern) {
    const paths = await glob(pattern + "/elm.json", {
        realpath: true,
    });
    const manifests = await Promise.all(paths.map((path) => getManifest(path)));
    let releases = manifests.reduce((releases, pkg) => {
...
function buildDocs(manifest, dir, elm, clean = true, verbose = false) {
...
function buildPackageDocs(dir, elm, clean) {
...
function buildApplicationDocs(manifest, dir, elm, clean, verbose) {
    ...
    let exposedModules = getExposedModules(pkg["exposed-modules"]);
...
function importModules(srcDir, dstDir, verbose) {
...
class DocServer {
    constructor(options) {
        const { address = "127.0.0.1", dir = ".", port = 8000, browser = true, reload = true, debug = false, verbose = false, initial = false, } = options || {};
        this.options = {
            address,
            browser,
            debug,
        ...
        info(chalk.bold(`elm-doc-preview ${version}`), `using elm ${this.elmVersion}`);
        if (this.manifest && this.manifest.name && this.manifest.version) {
            info("Previewing", chalk.magenta(`${this.manifest.name} ${this.manifest.version}`), `from ${this.options.dir}`);
        }
        else {
            info(`No package or application found in ${this.options.dir},`, "running documentation server only");
        }
        this.setupWebServer();
    ...
    setupWebServer() {
        ...
        this.app.get("/search.json", (_req, res) => {
            Promise.all([`${this.elmCache}/*/*/*`, "."].map((pattern) => searchPackages(pattern)))
                .then((packagesArray) => {
        ...
        this.app.get("/packages/:author/:project/:version/docs.json", (req, res) => {
            ...
            if (this.manifest && fullname(this.manifest) === name) {
                res.json(buildDocs(this.manifest, ".", this.elm, !this.options.debug, this.options.verbose));
            }
            else {
        ...
        if (this.manifest) {
            this.app.use(`/source/${fullname(this.manifest)}`, express.static(".", { setHeaders: setHeaders }), serveIndex(".", { icons: true }));
        }
        // Serve cached packages source
        this.app.use("/source", express.static(this.elmCache, { setHeaders: setHeaders }), serveIndex(this.elmCache, { icons: true }));
        // default route
    ...
    setupFilesWatcher() {
        ...
        watcher
            ...
            if (this.manifest && this.options.initial) {
                buildDocs(this.manifest, ".", this.elm, !this.options.debug, this.options.verbose);
            }
        });
    }
    onChange(filepath, sendDocsDebounced) {
    ...
    sendDocs() {
        if (this.manifest &&
            this.manifest.name &&
            this.manifest.name.includes("/")) {
            const docs = buildDocs(this.manifest, this.options.dir, this.elm, !this.options.debug, this.options.verbose);
            const [author, project] = this.manifest.name.split("/", 2);
            info("  |>", "sending Docs");
            this.broadcast({
                type: "docs",
    ...
    listen() {
        return this.app.listen(this.options.port, this.options.address, () => {
            ...
            info(chalk.blue("Browse"), chalk.bold.green(`http://localhost:${this.options.port.toString()}`), chalk.blue("to see your documentation"));
        });
    }
    make(filename) {
        if (this.manifest) {
